...
class S3FilesStore(object):
    ...
    def _get_boto_bucket(self):
        ...
        c = S3Connection(self.AWS_ACCESS_KEY_ID,
    ...
    def persist_file(self, path, buf, info, meta=None, headers=None):
        ...
        if headers:
            h.update(headers)
        buf.seek(0)
        return threads.deferToThread(k.set_contents_from_string,
                                     buf.getvalue(),
...
class FilesPipeline(MediaPipeline):
    ...
    def media_to_download(self, request, info):
        def _onsuccess(result):
            ...
            log.msg(format=('File (uptodate): '
                    'Downloaded %(medianame)s from %(request)s'
    ...
    def media_downloaded(self, response, request, info):
        ...
        if response.status != 200:
            log.msg(format=('File (code: %(status)s): '
                            'Error downloading file from %(request)s '
                            'referred in <%(referer)s>'),
                    level=log.WARNING, spider=info.spider,
        ...
        if not response.body:
            log.msg(format=('File (empty-content): '
                            'Empty file from %(request)s '
                            'referred in <%(referer)s>: no-content'),
                    level=log.WARNING, spider=info.spider,
                    request=request, referer=referer)
            raise FileException('empty-content')

        status = 'cached' if 'cached' in response.flags else 'downloaded'
        log.msg(format=('File (%(status)s): '
                        'Downloaded file from %(request)s '
                        'referred in <%(referer)s>'),
                level=log.DEBUG, spider=info.spider,
        ...
        try:
            ...
            whyfmt = ('File (error): ',
                      'Error processing file from %(request)s '
                      'referred in <%(referer)s>: %(errormsg)s')
            log.msg(format=whyfmt, level=log.WARNING, spider=info.spider,
                    request=request, referer=referer, errormsg=str(exc))
            raise
        except Exception as exc:
            whyfmt = ('File (unknown-error):'
                      'Error processing file from %(request)s '
                      'referred in <%(referer)s>')
            log.err(
                None,
                whyfmt % {'request': request, 'referer': referer},
                spider=info.spider
            )
